...
// --- Part 1: Create 'new_news' table if it doesn't exist ---
// The 'news_datetime' column is VARCHAR to store the exact string from JavaScript.
// If you need a true MySQL DATETIME for sorting/filtering, you would need
// to parse and format the incoming Bangla datetime string in PHP.
$sql_news_table = "CREATE TABLE IF NOT EXISTS new_news (
...
if ($conn->query($sql_news_table) === TRUE) {
    ...
    // In a production environment, avoid echoing success messages here,
    // as they would interfere with the JSON response for the form submission.
} else {
    // If there's an error creating the table, stop and report it.
    echo json_encode(["success" => false, "message" => "Error creating news_news table: " . $conn->error]);
    $conn->close();
    exit();
}

// --- Part 2: Create 'posts' table if it doesn't exist ---
// This table will store content copied from news_heading.
...
if ($conn->query($sql_posts_table) === TRUE) {
    // Table created successfully or already exists.
} else {
    // If there's an error creating the posts table, stop and report it.
    echo json_encode(["success" => false, "message" => "Error creating posts table: " . $conn->error]);
    $conn->close();
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt_news = $conn->prepare("INSERT INTO new_news (news_image, news_heading, news_description, news_category, news_datetime) VALUES (?, ?, ?, ?, ?)");

    // Check if the statement preparation was successful
    if ($stmt_news === false) {
        echo json_encode(["success" => false, "message" => "Failed to prepare news_news statement: " . $conn->error]);
        $conn->close();
        exit();
    }

    // Bind parameters for new_news
    // 'sssss' indicates all five parameters are strings.
    $stmt_news->bind_param("sssss", $image_path, $heading, $description, $category, $newsDateTime);

    // Execute the statement for new_news
    if ($stmt_news->execute()) {
        // If news_news insertion is successful, proceed to insert into 'posts' table
        $stmt_posts = $conn->prepare("INSERT INTO posts (content) VALUES (?)");

        if ($stmt_posts === false) {
            // Log or handle error, but don't exit immediately as news_news was successful
            error_log("Failed to prepare posts statement: " . $conn->error);
            echo json_encode(["success" => false, "message" => "New news article added, but failed to prepare posts insertion: " . $conn->error]);
        } else {
            // Bind parameters for posts table
            $stmt_posts->bind_param("s", $heading);

            // Execute the statement for posts
            if ($stmt_posts->execute()) {
                echo json_encode(["success" => true, "message" => "New news article and post added successfully!"]);
            } else {
                // Error inserting into posts table
                error_log("Error inserting into posts table: " . $stmt_posts->error);
                echo json_encode(["success" => false, "message" => "New news article added, but failed to add to posts: " . $stmt_posts->error]);
            }
            $stmt_posts->close(); // Close the posts statement
        }
    } else {
        echo json_encode(["success" => false, "message" => "Error inserting data into news_news: " . $stmt_news->error]);
    }
